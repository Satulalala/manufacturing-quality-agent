...
- **自然语言提问**：自动识别问题中的日期范围和产线（如"请分析 2026-01-01 到
  2026-01-15 A产线的不良率异常"）。
...
## 评测结果（2026-08-08，demo_records.csv，2400 条）
...
**失败案例（已知限制，如实记录）**
1. 车型问题不解析："ID.4 车型" 中的车型被忽略，只按日期/产线分析。
2. 中文日期不解析："1月1日" 无法识别，只认 `20xx-xx-xx` 格式。
3. 复合查询截断："A产线和B产线" 只取第一个产线 A。
...
1. 将 `parse_question()` 替换为 LLM 结构化输出，支持车型、中文日期、复合查询。
